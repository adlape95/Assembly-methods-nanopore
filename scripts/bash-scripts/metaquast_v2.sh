...
###########################
# This script has been used to evaluate the draft-assemblies with the program MetaQUAST https://github.com/ablab/quast .
...
for ((i=0; i<=${#INFO[@]}-1; i+=3)); do
	for ((j=0; j<=${#Assemblers[@]}-1; j++)); do
		...
		if [ -d ./$FOLDER ]; then
			echo -e "\n\n"metaquast ${Assemblers[$j]} ${INFO[$i]} ${INFO[$i+2]} | tr '[:lower:]' '[:upper:]'
			echo
			cd $FOLDER
		else
		...
		if [ -s $FILE ]; then
			...
			# Execute MetaQUAST.
			metaquast.py -o metaquast -r $Path_reference -t 16 --ambiguity-usage one --fragmented $FILE
		else
